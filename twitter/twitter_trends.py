...
# 5일 데이터보기
for i in range(5):
# def extract_twt_data(
#     twt_auth, 
#     twt_query, 
#     execute_date,
#     output):
    # 오늘 날짜 뺴고 체크
    START = (now - timedelta(days=i+2)).strftime("%Y-%m-%d")
    END = (now - timedelta(days=i+1)).strftime("%Y-%m-%d")
    ...
    #%%
    # 날짜별로 파일 구분하고 데이터 얻어오기
    with open(f'twitter_raw_data-{START}.file','w') as a:
        # 나중에 dump하기 쉽게 한 줄로 만들기
        for one_data in raw_data:
            a.write(json.dumps(one_data))
            a.write('\n')
